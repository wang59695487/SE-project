...
def market(request):
    ...
    info_today['yest_close'] = info_yesterday['close']
    info_today['rate'] = (info_today.close - info_today.yest_close) / info_today.yest_close

    info_today = info_today.sort_values('rate', ascending=False)

    for code, stk in info_today[info_today['rate'] >= 0].iloc[:20].iterrows():
        result['top_surge'].append({'code': int(code), 'name': stk['name'], 'close': stk['close'], 'rate': stk['rate']})
    for code, stk in info_today[info_today['rate'] <= 0].iloc[:-21:-1].iterrows():
        result['top_decline'].append(
            {'code': int(code), 'name': stk['name'], 'close': stk['close'], 'rate': stk['rate']})
    ...
    for index, row in industries.iterrows():
        ...
        industries.loc[index, 'surged'] = len(stk_info[stk_info['rate'] > 0])
        industries.loc[index, 'declined'] = len(stk_info[stk_info['rate'] < 0])
    ...
    for _, row in industries[industries['rate'] >= 0].iloc[:10].iterrows():
        result['top_industries'].append({'name': row['name'], 'total': row['total'], 'count': row['surged'], 'rate': row['rate']})
    ...
    result['surged'] = len(info_today[info_today.rate > 0])
    result['balanced'] = len(info_today[info_today.rate == 0])
    result['declined'] = len(info_today[info_today.rate < 0])
...
def stock_list(request):
    ...
    info_today.loc[:, 'yest_close'] = info_yesterday['close']
    info_today.loc[:, 'rate'] = (info_today.close - info_today.yest_close) / info_today.yest_close

    result = []
    for code, row in info_today.dropna().iterrows():
        result.append((int(code), row['name'], row['open'], row['low'], row['high'], row['close'], row['yest_close'],
                       row['rate'], row['volume']))
